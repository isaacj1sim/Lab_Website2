...
function loadStatsPage(){
	console.log("in loadStatsPage");
	var table = document.getElementById("stats_table");
	var row_counter;
	var col_counter;
	var cell_value;
	var winner;
	var buffs = "CU Boulder";
	var opTeam;
	var opTScore;
	var bufScore;
	var bufWin_Counter = 0;
	var opTWin_Counter = 0;
	...
	for(row_counter = 0; row_counter < table.rows.length; row_counter++){
		...
		for(col_counter = 0; col_counter < table.rows[row_counter].cells.length; col_counter++){
			...
			if(col_counter == 1){
				opTeam = table.rows[row_counter].cells[col_counter].innerHTML;
			}
			else if(col_counter == 2){
				bufScore = table.rows[row_counter].cells[col_counter].innerHTML;
				bufScore = parseInt(bufScore);
			}
			else if(col_counter == 3){
				opTScore = table.rows[row_counter].cells[col_counter].innerHTML;
				opTscore = parseInt(opTScore);
			}
			else if(col_counter == 4){
				if(opTScore > bufScore){
					table.rows[row_counter].cells[col_counter].innerHTML = opTeam;
					opTWin_Counter++;
					console.log(opTeam);
				}
				else if(bufScore > opTScore){
	...
	document.getElementById("wins").innerHTML = bufWin_Counter;
	document.getElementById("losses").innerHTML = opTWin_Counter;
}
